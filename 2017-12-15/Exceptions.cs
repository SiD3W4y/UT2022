...
internal class Tests
{
    ...
    {
        AssertException<ArgumentException>(() => Base.Fibonacci(-30));
        AssertException<ArgumentException>(() => Base.Fibonacci(-1));
        
        Assert(Base.Fibonacci(0)  == 0);
        Assert(Base.Fibonacci(1)  == 1);
        Assert(Base.Fibonacci(10) == 55);
    }

    [Test]
    public static void TestConversions()
    {
        AssertException<ArgumentException>(() => Base.DegToRad(181));
        AssertException<ArgumentException>(() => Base.DegToRad(-181));
        AssertException<ArgumentException>(() => Base.DegToRad(360));
        AssertException<ArgumentException>(() => Base.RadToDeg(3.15f));
        AssertException<ArgumentException>(() => Base.RadToDeg(-3.15f));
        AssertException<ArgumentException>(() => Base.RadToDeg(-10.0f));
        
        Assert(Base.DegToRad(0) == 0);
        Assert(Base.RadToDeg(0) == 0);

        Assert(Math.Abs(Base.RadToDeg(0.5236f) - 30) < .001);
        Assert(Math.Abs(Base.DegToRad(30) - 0.5236) < .001);
    }
    ...
    public static void TestPower()
    {
        ...
        for (int i = 0; i < 40; i++)
        {
            ...
            var user = Base.Pow(b, a);
            var lib  = Math.Pow(b, a);
            
            Assert(Math.Abs(Base.Pow(b, a) - (float)Math.Pow(b, a)) < 0.1);
        }
        
        // test overflows
        AssertException<OverflowException>(() => Base.Pow(10, 101));
    }
    #endregion
